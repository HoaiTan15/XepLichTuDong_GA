...
class GeneticAlgorithm:
    ...
    def create_individual(self):
        ...
        for _, row in self.df.iterrows():
            schedule.append({
                ...
                "timeslot": generate_preferred_timeslot()
            })
        return schedule
    ...
    def run(self):
        ...
        for gen in range(self.generations):
            ...
            # FIX: Tăng early stopping lên 40 thế hệ
            if stagnant_count > 40:
                print(f"⏹️ Dừng sớm ở thế hệ {gen} (không cải thiện trong 40 thế hệ)")
                break
            ...
            # FIX: Tournament selection từ TOÀN BỘ population
            while len(new_pop) < self.population_size:
                # Tournament selection: chọn từ toàn bộ population (không chỉ top 50%)
                tournament1 = random.sample(scored, min(3, len(scored)))  #  FIX
                tournament2 = random.sample(scored, min(3, len(scored)))  # FIX
                ...
                #  FIX: Tăng base mutation rate lên 0.15
                base_mutation = 0.15  #  Tăng từ 0.1 lên 0.15
                mutation_rate = base_mutation + (stagnant_count * 0.02)
